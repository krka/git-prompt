...
def generate_unified_detailed_report(suites_results, output_path):
    """Generate a unified HTML report combining all test suites

    Args:
        suites_results: List of (suite_name, test_results) tuples
...
<head>
    ...
    <title>Git Prompt Test Results - All Suites</title>
...
<body>
    <h1>Git Prompt Test Results - All Suites</h1>
"""

    # Calculate overall stats across all suites
    all_results = []
    for suite_name, test_results in suites_results:
    ...
    <div class="{summary_class}">
        <strong>Overall Summary:</strong> {passed}/{total} passed ({pass_rate:.1f}%) across {len(suites_results)} test suite(s)
...
"""
    ...
    for suite_name, test_results in suites_results:
        ...
        suite_header_class = "suite-header" if suite_failed == 0 else "suite-header failed"
...
def generate_detailed_report(test_results, output_path):
    """Generate a detailed HTML report with all test steps and results (single suite version)"""
    # For single suite, wrap in list format and call unified version
    suites_results = [("Test Results", test_results)]
    return generate_unified_detailed_report(suites_results, output_path)
...
"""
    ...
    # Add each group
    for group_key in ['basic', 'working-tree', 'branches', 'detached', 'upstream', 'stash', 'in-progress', 'bisect']:
...
def run_test_suite(test_file, git_prompt_path, verbose=False, replace_expected=False, generate_reports=True):
    """Run all tests from YAML file

    Args:
        test_file: Path to YAML test file
        git_prompt_path: Path to git-prompt binary
        verbose: Print verbose output
        replace_expected: Replace expected values with actual output
        generate_reports: Generate individual reports (disable when running multiple suites)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Generate reports if requested (disabled when running multiple suites)
        if generate_reports and test_results:
...
def main():
    ...
    all_success = True
    suites_results = []  # Collect (suite_name, test_results) tuples

    # Determine if we should generate individual reports or unified
    generate_individual = len(test_files) == 1
    ...
    for test_file in test_files:
        ...
        success, test_results = run_test_suite(
            test_file,
            git_prompt_path,
            verbose=args.verbose,
            replace_expected=args.replace_expected,
            generate_reports=generate_individual
        )

        if not success:
            all_success = False

        # Collect results for unified reporting
        suite_name = test_file.stem.replace('test-', '').replace('_', ' ').title()
        if suite_name == 'Test Cases':  # Legacy test_cases.yaml
    ...
    if len(test_files) > 1:
        ...
        # Generate the full examples report (all tests from all suites)
        examples_full_path = script_dir / 'examples.html'
        generate_html_report(all_test_results, examples_full_path, examples_only=False)

        # Generate the filtered examples report (only tests marked as examples)
        examples_doc_path = script_dir / 'examples-doc.html'
        generate_html_report(all_test_results, examples_doc_path, examples_only=True)

        # Generate the examples snippet for embedding (only examples)
        examples_snippet_path = script_dir / 'examples-snippet.html'
        generate_examples_snippet(all_test_results, examples_snippet_path)

        # Generate the unified detailed test results report (split by suite)
        detailed_path = script_dir / 'test-results.html'
        generate_unified_detailed_report(suites_results, detailed_path)
        print(f"{Colors.BLUE}Unified test results: {detailed_path}{Colors.RESET}")

        # Generate a combined text report
        text_path = script_dir / 'test-results.txt'
        generate_text_report(all_test_results, text_path)
        print(f"{Colors.BLUE}Text report: {text_path}{Colors.RESET}")
